...
public class ClientHandler implements Runnable {
    ...
    private Message handleAdminCancelAuction(Message req) {
        ...
        var payload = req.getPayload(AdminCancelPayload.class); Auction auction = AuctionManager.getInstance().findById(payload.auctionId);
        if (auction == null) return Message.error(MessageType.ERROR, "Không tìm thấy phiên.");
    ...
    private AuctionDto toDto(Auction a) {
        ...
        dto.history = a.getBidHistory().stream().map(b -> {
            ...
            entry.bidderName = b.getBidder().getUsername();
            entry.amount = b.getAmount();
            entry.time = b.getTimestamp().format(DateTimeFormatter.ofPattern("HH:mm:ss dd/MM/yyyy"));
            return entry;
